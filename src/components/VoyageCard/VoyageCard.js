...
const VoyageCardCreator = ({ leftPanel, rightPanel, action, footer, team, backgroundColor }) => (
    ...
    <VoyageCardCreator
      ...
      action={() =>
        alreadyApplied ? (
          ...
          <Action routeId={voyage.id} userStatus={user.status} />
        )
      }
    />
  );
...
export const ApplyForAVoyageCard = () => (
  <VoyageCardCreator
    backgroundColor='#EFEFEF'
    leftPanel={() => (
      <div className='card-circle card--no-voyage'>
...
<span className='card-label'>voyage</span>
    ...
    rightPanel={() => (
      <div className='card-info--no-voyage'>SORRY, LOOKS LIKE YOU AREN'T PART OF A VOYAGE YET!</div>
    )}
    action={() => (
...
<span className='card-label'>voyage</span>
    ...
    rightPanel={() => (
      <div className='card-info--no-voyage'>
...
export const CurrentVoyageCardWithTeam = ({ voyage, team }) => {
  const currentStandUp =
    team.standups && team.standups.filter(standup => standup.expiration > Number(new Date()));
  return (
    <VoyageCardCreator
      leftPanel={() => <Badge voyage={voyage} />}
      rightPanel={() => <Info startDate={voyage.start_date} endDate={voyage.end_date} />}
