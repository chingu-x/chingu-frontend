...
class TeamHelp extends React.Component {
  constructor(props) {
    ...
    if (persistedData) {
      const {
        form_data: { request_subtype, team_id },
  ...
  submitRequest = form_data => {
    const mutation = gql`
      mutation submitTeamHelpRequest($help_request_data: TeamHelpRequestCreateInput!) {
        teamHelpRequestCreate(help_request_data: $help_request_data) {
          id
        }
      }
    `;
