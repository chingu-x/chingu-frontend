...
class Header extends React.Component {
  ...
  renderAvatar = avatar => (
    <PopupMenu className='header-dropdown'>
      <img
        ...
        className='avatar'
        src={avatar || require('../../assets/blank image.png')}
        alt='user avatar'
      />
      <div className='header-dropdown-content'>
        <Link to='/newsfeed'>Newsfeed</Link>
        <Link to='/voyage' onMouseOver={() => client.query({ query: voyagesQuery })}>
          Voyage Portal
        </Link>
  ...
  render() {
    ...
    return (
      <Fragment>
        <Modal ref='loginModal'           background='transparent'
        >
          <GithubLoginModal />
        </Modal>
          ...
          <div className='header-container'>
            ...
            <div className='header-right'>
              ...
              {!localStorage.token && !user && (
                <div onClick={this.openLoginModal} className='header-btn'>
                  LOG IN
                </div>
              )}
            </div>
...
export default withRouter(props =>
  !localStorage.token ? (
    ...
    <Request {...props} component={Header} query={userBaseQuery} />
  ),
);
