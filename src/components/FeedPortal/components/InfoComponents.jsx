...
const InfoComponents = ({ team }) => {
	...
	let infoObjects = [
		{
			label: 'Voyage',
			data: `${cohort.title} - ${dateFormatter(cohort.start_date)} - ${dateFormatter(
				cohort.end_date
			)}`,
		},
	...
	return infoObjects.map((info, idx) => {
		...
		switch (info.label) {
			case 'Tier':
				data = (
					<span key={idx} className="tier-icon">
						{info.data}
					</span>
				);
				break;
			...
			case 'TechStack':
				data =
					info.data &&
					info.data.map((tech, idx) => {
						return (
							<div key={idx} className="team-card-techstack">
								{tech.name}
							</div>
						);
					});
